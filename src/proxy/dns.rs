...
pub async fn run_dns_proxy(
    ...
    println!(
        "[info] dns proxy listening on {} -> {} ({})",
        listen,
        upstream,
        if via_socks { format!("via socks {socks5}") } else { "direct".to_string() }
    );
    ...
    loop {
        ...
        let response = query_upstream(&req, upstream, socks5, via_socks).await;
        match response {
            Ok(resp) => {
...
async fn query_upstream(
    ...
    if via_socks {
        return socks5::udp_query(socks5, upstream, payload).await;
    }
