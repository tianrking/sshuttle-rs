...
- `platform backends`
  ...
  - Windows backend (planned: WinDivert/WFP)
...
- Windows backend intentionally marked as planned, not yet implemented.
...
- Milestone B: nftables backend + IPv6 original-dst path.
- Milestone C: UDP/DNS interception strategy (TPROXY/WFP-compatible abstraction).
- Milestone D: Windows backend (WinDivert/WFP) with connection tracking.
- Milestone E: optional SSH transport mode for sshuttle compatibility (client/server mux in Rust).
